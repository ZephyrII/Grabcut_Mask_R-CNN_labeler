...
class Detector:
    ...
    def __init__(self, path_to_model, path_to_pole_model, camera_matrix):
        ...
        self.offset = (0, 0)
        self.scale = 1.0
        self.charger_to_slice_ratio = 0.75
        self.detections = []
    ...
    def detect(self, frame, gt=None):
        ...
        if self.best_detection is not None:
            ...
            print("width", width)
            if width > self.slice_size[1]*self.charger_to_slice_ratio: # or height > self.slice_size[0]*self.charger_to_slice_ratio:
                self.scale = self.slice_size[1]/width*self.charger_to_slice_ratio #min(self.slice_size[1]/width*self.charger_to_slice_ratio, self.slice_size[0]/height*self.charger_to_slice_ratio)
            # if width < self.slice_size[1]*self.charger_to_slice_ratio and height < self.slice_size[0]*self.charger_to_slice_ratio:
            #     self.scale = 1.0
            # else:
            #     self.scale = min(self.slice_size[1]/width*self.charger_to_slice_ratio, self.slice_size[0]/height*self.charger_to_slice_ratio)
        return
    ...
    def detect_pole(self, small_frame):
        ...
        if len(detections) > 0:
            ...
            y_off = int(np.max((0, np.min((best_detection['abs_rect'][1]-self.slice_size[0]/3, self.frame_shape[0]-self.slice_size[0])))))
            x_off = int(np.max((0, np.min((best_detection['abs_rect'][0]-self.slice_size[1]/3, self.frame_shape[1]-self.slice_size[1])))))
            self.offset = (y_off, x_off)
            self.init_det = False
    ...
    def get_best_detections(self):
        ...
        y_off = int(np.max((0, np.min((y1-self.slice_size[0]/3, self.frame_shape[0] - self.slice_size[0])))))
        x_off = int(np.max((0, np.min((x1-self.slice_size[1]/3, self.frame_shape[1] - self.slice_size[1])))))
        self.offset = (y_off, x_off)
        return (x2-x1, y2-y1)
