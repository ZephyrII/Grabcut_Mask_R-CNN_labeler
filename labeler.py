...
class DetectorNode:
    ...
    def __init__(self):
        ...
        self.output_directory = '/root/share/tf/dataset/5_point_new/'
        ...
        self.rosbag_name = "_2020-05-25-13-20-07.bag" #54-80 skipped
        command = "rosbag play -r 0.3 -s 15 /root/share/tf/dataset/5_point_new/" + self.rosbag_name
        ...
        while not rospy.is_shutdown():
            if self.image is not None:
                ...
                if k == ord('q'):
                    ...
                    self.bag_process.send_signal(signal.SIGKILL)
                    exit(0)
        rospy.spin()
    ...
    def save_mask(self, stamp, mask, score):
        if mask is not None:
            res = 1.0
            if abs(self.keypoints[0][1] - self.keypoints[2][1]) > self.slice_size[1] / 2:  #TODO: verify keypoints on corners
                res = self.slice_size[1] / 2 / abs(self.keypoints[0][1] - self.keypoints[2][1])
                # print(res)
